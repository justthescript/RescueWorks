...
def create_category(cat_in: schemas.ExpenseCategoryCreate, db: Session = Depends(get_db), user=Depends(require_any_role([ROLE_BILLING_MANAGER, ROLE_ADMIN, ROLE_SUPER_ADMIN]))):
...
def list_categories(db: Session = Depends(get_db), user=Depends(get_current_user)):
    return db.query(models.ExpenseCategory).filter(models.ExpenseCategory.org_id == user.org_id).all()


@router.post("/", response_model=schemas.Expense)
def create_expense(exp_in: schemas.ExpenseCreate, db: Session = Depends(get_db), user=Depends(require_any_role([ROLE_BILLING_MANAGER, ROLE_ADMIN, ROLE_SUPER_ADMIN]))):
    if user.org_id != exp_in.org_id:
        raise HTTPException(status_code=400, detail="User org mismatch")
    cat = db.query(models.ExpenseCategory).filter(models.ExpenseCategory.id == exp_in.category_id, models.ExpenseCategory.org_id == user.org_id).first()
    if not cat:
