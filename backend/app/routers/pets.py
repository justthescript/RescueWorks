...
def create_pet(
    ...
    user=Depends(
        require_any_role([ROLE_PET_COORDINATOR, ROLE_ADMIN, ROLE_SUPER_ADMIN])
    ),
):
    org = (
        db.query(models.Organization)
        .filter(models.Organization.id == pet_in.org_id)
        .first()
    )
    if not org:
...
def assign_foster(
    ...
    if not foster_user.is_active:
        raise HTTPException(
            status_code=400, detail="Cannot assign pet to inactive user"
        )
